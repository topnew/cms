 ...
 * The MIT License (MIT) Copyright (c) 1998-2015, Topnew Geo, topnew.net
...
// the folloing func is going to delete
function _cms_conn_del($db = '', $eng = '') {
    global $pin;
    if (!$eng && isset($pin['eng'])) $eng = $pin['eng'];
    if (!$eng || !in_array($eng, array('mysql','pgsql','sqlite'))) $eng = 'mysql';
    $pin['eng'] = $eng;
    $conn = isset($pin['conn']) ? $pin['conn'] : cms_conn_init();
    $conn = $conn[$eng];
    if (isset($conn['enc']) && $conn['enc']) $conn['p'] = cms_dec($conn['p'], 1);
    if (!$db) $db = isset($pin['db']) ? $pin['db'] : $conn['db'];
    if ('sqlite' == $eng) return new PDO('sqlite:' . $db);
    $pdo = $eng .':host=' . $conn['h'] . ';dbname=' . $db;
    $port = isset($conn['port']) ? $conn['port'] : 0;
    if ('cubrid' == $eng && !$port) $port = 30000;
    if (('mysql' == $eng && $port && $port<>3306)
     || ('pgsql' == $eng && $port && $port<>5432)
     || 'cubrid' == $eng) {
        $pdo .= ';port=' . $port;
    }
    $options = ('mysql' == $eng && isset($conn['char']) && 'utf8' == $conn['char']) ? array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8') : array();
...
/**
 * $sql = '... where col='. cms_para('col', $val, $para)
 * cms_row($sql, $para)
 */
function _cms_para_del($key = '', $val = '', &$para) {
    $key = ':'. $key;
    $para[$key] = $val;
    return $key;
}
function _cms_sidu($cmd = '', $tab = '', $arr = array(), $where = '', $update_para = array()) {
    $cmd = strtolower($cmd);
    if (in_array($cmd, array('arr', 'row', 'rows', 'val'))) {
        $func = 'cms_' . $cmd;
        return $func($tab, $arr, $where);
    }
    if ('delete' == $cmd) return cms_run('DELETE FROM '. $tab ."\n". $where, $arr);
    if ('save' == $cmd) $cmd = cms_val('SELECT 1 FROM '. $tab .' ' . $where .' LIMIT 1', $update_para) ? 'update' : 'insert';
    $para = $data = array();
    if ('insert' == $cmd || 'replace' == $cmd) {
        foreach ($arr as $k => $v) {
            if (is_null($v)) $v = 'NULL';
        ...
        $sql = strtoupper($cmd) .' INTO '. $tab .'('. implode(', ', array_keys($arr)) .")\nVALUES(". implode(', ', $data) .')';
        $pg_insert_id = ('insert' <> $cmd || substr($where, 0, 8) <> 'last_id:') ? '' : substr($where, 8);
        return cms_run($sql, $para, $pg_insert_id);
    }
    ...
    $sql = 'UPDATE '. $tab ."\nSET ". implode(', ', $data) ."\n". $where;
    if (!$where || !is_array($update_para) || !count($update_para)) return cms_run($sql, $para);
    foreach ($update_para as $k => $v) {
        $k .= '_where';
        if (!isset($para[$k])) $para[$k] = $v;
        else $para[$k .'_'. rand()] = $v;//possible bug not merely impossible
    }
    return cms_run($sql, $para);
}
